...
class RvnotesAdapter: ListAdapter<Note, RvnotesAdapter.NotesViewHolder>(DiffUtilCallback()) {
    ...
    inner class NotesViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        // regference to card for changing the colour of it
        val parent : MaterialCardView = contentBinding.notesItemLayoutParent
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, position: Int) {
        getItem(position).let {note->
            holder.apply {
                parent.transitionName = "recylerView_${note.id}"
